...
table(Predicted = predicted_p, Actual = train_data$Direction)


##Threshold -> maximizes the TPR(1-FPR)
cc = coords(rc, "all", ret = c("sensitivity", "specificity", "threshold"), transpose = FALSE)
TPR = cc$sensitivity
FPR = 1-cc$specificity
qant = TPR*(1-FPR);qant
opt = which(qant == max(qant))
best_row = cc[opt,]; best_row
pred_res = ifelse(p >= best_row$threshold,1,0); pred_res
confusion.matrix = table(Predicted = pred_res, Actual = train_data$Direction); confusion.matrix
##ROC curve
